...
internal class CustomDrawer protected constructor(private val mContext: Context, private val mDrawerLayout: DrawerLayout, private val mDrawerList: ListView, private val mToolbar: Toolbar?) : FactoryInterface {

    //class of creating and populating navigation drawer
    @Override
    fun doTask(): ActionBarDrawerToggle? {

        val activity = mContext as AppCompatActivity
        val rowItems = ArrayList()
        //get the arrays from strings
        val menuTitles = mContext.getResources().getStringArray(R.array.titles)
        val menuIcons = mContext.getResources().obtainTypedArray(R.array.icons)
        //create the drawer
        var drawerToggle: ActionBarDrawerToggle? = null
        if (mToolbar != null) {
            drawerToggle = object : ActionBarDrawerToggle(activity, mDrawerLayout,
        ...
        val startingListNum = 0
        //create the drawer list names and icons
        for (i in startingListNum until menuTitles.size) {
            val items = RowItem(menuTitles[i], menuIcons.getResourceId(i, -1))
            rowItems.add(items)

        }
        //sets drawer adapter
        val adapter = DrawerAdapter(mContext, rowItems)
        mDrawerList.setAdapter(adapter)
        mDrawerList.setOnItemClickListener(CustomDrawer.SlideitemListener())
        menuIcons.recycle()
    ...
    private fun updateDisplay(position: Int) {
        ...
        val tag = "search"
        val tabName: Array<String>? = null
        val tabTags: Array<String>? = null
        ...
        //Create tabs
        if (tabName != null) {
            (mContext as MainActivity).changeTabs(tabName, tabTags)
        }
        //Change to clicked Fragment
        if (fragment != null) {
            if ((mContext as MainActivity).getFragmentManager().findFragmentByTag(tag) == null) {
    ...
    internal inner class SlideitemListener : ListView.OnItemClickListener {
